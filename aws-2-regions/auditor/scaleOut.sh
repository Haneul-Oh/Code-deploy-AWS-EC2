...
for (( i=0; i <${sizeClient}; i++ )); do
    sed -i "$((${col}+5+${i}))s/.*/      - \"aggregator$((${i}+1)).edgechain0.com:${clients_seo[i]}\"/g" ${fname}
done
...
for (( i=0; i <${sizeAuditor_sin}; i++ ))
do
    fname="./auditchain_dis_sin_${i}.yaml"
    cp "./auditchain_dis.yaml" ${fname}
    j=$((${i}+${sizeAuditor_seo}+${sizeAuditor_vir}+${sizeAuditor_fra}))
    sed -i "15s/.*/  auditor${j}:/g" ${fname}
    sed -i "16s/.*/    container_name: auditor${j}/g" ${fname}
    sed -i "42s/.*/      - CORE_OPERATIONS_LISTENADDRESS=auditor${j}:9443/g" ${fname}
    sed -i "48s/.*/      - EDGECHAIN_NODE_NAME=auditor$((${j}+1)).edgechain0.com/g" ${fname}
    ...
    sed -i "65s/.*/      - NUM_QUORUM=$((${size_f}*2+1))/g" ${fname}
    sed -i "68s/.*/      - COMMITTEESIZE=${sizeAuditor_total}/g" ${fname}
    sed -i "72s/.*/      - NODE_IP=${auditors_sin[i]}/g" ${fname}
    sed -i "74s/.*/      - NODE_IP_LIST=${nodeIP_list}/g" ${fname}
...
for (( i=0; i <${sizeAuditor_cal}; i++ ))
do
    fname="./auditchain_dis_cal_${i}.yaml"
    cp "./auditchain_dis.yaml" ${fname}
    j=$((${i}+${sizeAuditor_seo}+${sizeAuditor_vir}+${sizeAuditor_fra}+${sizeAuditor_sin}))
    sed -i "15s/.*/  auditor${j}:/g" ${fname}
    sed -i "16s/.*/    container_name: auditor${j}/g" ${fname}
    sed -i "42s/.*/      - CORE_OPERATIONS_LISTENADDRESS=auditor${j}:9443/g" ${fname}
    sed -i "48s/.*/      - EDGECHAIN_NODE_NAME=auditor$((${j}+1)).edgechain0.com/g" ${fname}
    ...
    sed -i "65s/.*/      - NUM_QUORUM=$((${size_f}*2+1))/g" ${fname}
    sed -i "68s/.*/      - COMMITTEESIZE=${sizeAuditor_total}/g" ${fname}
    sed -i "72s/.*/      - NODE_IP=${auditors_cal[i]}/g" ${fname}
    sed -i "74s/.*/      - NODE_IP_LIST=${nodeIP_list}/g" ${fname}
...
for (( i=0; i <${sizeAuditor_ire}; i++ ))
do
    fname="./auditchain_dis_ire_${i}.yaml"
    cp "./auditchain_dis.yaml" ${fname}
    j=$((${i}+${sizeAuditor_seo}+${sizeAuditor_vir}+${sizeAuditor_fra}+${sizeAuditor_sin}+${sizeAuditor_cal}))
    sed -i "15s/.*/  auditor${j}:/g" ${fname}
    sed -i "16s/.*/    container_name: auditor${j}/g" ${fname}
    sed -i "42s/.*/      - CORE_OPERATIONS_LISTENADDRESS=auditor${j}:9443/g" ${fname}
    sed -i "48s/.*/      - EDGECHAIN_NODE_NAME=auditor$((${j}+1)).edgechain0.com/g" ${fname}
    ...
    sed -i "65s/.*/      - NUM_QUORUM=$((${size_f}*2+1))/g" ${fname}
    sed -i "68s/.*/      - COMMITTEESIZE=${sizeAuditor_total}/g" ${fname}
    sed -i "72s/.*/      - NODE_IP=${auditors_ire[i]}/g" ${fname}
    sed -i "74s/.*/      - NODE_IP_LIST=${nodeIP_list}/g" ${fname}

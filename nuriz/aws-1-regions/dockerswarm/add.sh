...
AUDITOR_VIR_KEY="~/keystore/cloudvir.pem"
...
for ip in ${new[@]}; do
    ...
    sleep 2
	result=$(ssh -i ${CLIENT_KEY} ubuntu@${ip} "docker swarm join --token ${token} ${BSP}:2377")
    if [[ "${result}" != *${completion}* ]]; then
        dockerswarmresult+=("aud-vir: ${ip}:")
    fi
done

echo "docker swarm leave -f && docker swarm join --token ${token} ${BSP}:2377"
